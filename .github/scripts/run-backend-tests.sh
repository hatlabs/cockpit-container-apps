...
output=$(docker compose -f docker/docker-compose.devtools.yml run --rm --user root devtools bash -c "cd backend && UV_SYSTEM_PYTHON=1 uv sync --extra dev && uv run pytest" 2>&1)
echo "$output"
